 ...
 */
function htmlToDitaTable(html: string, hardLineBreak: RegExp, eventLogger: simpleLogger): string
{
    try
    {
        ...
        $('tbody tr').each((i, elem) => 
        {
            ...
            $(elem)
                ...
                .each((j, cell) =>
                {
                    ...
                    const wrappedContent = checkAndWrapPTags(cellContent, hardLineBreak);
 ...
 */
function checkAndWrapPTags(content: string, hardLineBreak: RegExp): string
{
    ...
    // Check for hard line breaks using the hardLineBreak regex
    if (hardLineBreak.test(content))
    {
        // Replace all hard line breaks with <p> tags
        return content.replace(hardLineBreak, '<p/>');
    }
